...
function setPage(id) {
    ...
    if (document.getElementById(id) != null)
        setActiveMenu(document.getElementById(id).getElementsByTagName("a"), extractPageName(hrefString));
}
...
$(document).ready(function () {
    ...
    $('nav li').filter(function () {
        return $.trim($(this).text()) == '' || ($(this).has('ul').length > 0 && $.trim($(this).find('ul').text()) == '')
    }).remove();

    $('nav').filter(function () {
        return !($(this).has('li').length)
    }).remove();

    // arcordion menu

    $('nav h4.collapsed').each(function () {
        var h = $(this);
        if (h.parent().find('li.active').length) {
            h.removeClass('collapsed');
        }
        else {
            h.parent().find('ul').hide();
        }
    });
    ...
    $('nav h4').click(function () {
        ...
        h.parent().find('ul').slideToggle('fast');
    });
    ...
    */
    ...
    $(window).scroll(function () {
        ...
        if ($('#wrapper > header').length > 0) {
            ...
            if (!$.browser.msie) { $('#wrapper > header').animate({ opacity: ($(document).scrollTop() <= 10 ? 1 : 0.7) }); }
        }
